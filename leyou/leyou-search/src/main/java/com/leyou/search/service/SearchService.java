...
public class SearchService {
    ...
    * */
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        //根据分类的id查询分类名称
        List<String> names = this.categoryClient.queryNamesByIds(Arrays.asList(spu.getCid2(), spu.getCid2(), spu.getCid3()));
        ...
        skus.forEach(sku -> {
            ...
            //获取sku中的图片，数据库中的图片可能是多张，多张是以“，”分割，所以也以逗号进行切割，返回图片数组，获取第一种图片
            map.put("image",StringUtils.isBlank(sku.getImages())?" ":StringUtils.split(sku.getImages(),",")[0]);
            skuMapList.add(map);
        });

        //根据spu中的cid3查询出所有的搜索规格参数
        //注意这里的参数列表要和我们的方法的参数列表一致
